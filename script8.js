...
function refleshjs(){
    let js8ArrayNumbers=[],tut=0,say=0;
    ...
    function arrayControl(caseNumber){
        if(js8ArrayNumbers.includes(caseNumber)== true) // üretilen sayıyı dizide kontrol eder
        {
            console.log("pass"); // dizide varsa pass geçer
        }else{
            js8ArrayNumbers.push(caseNumber); // sayı dizide yoksa ekler
            console.log("add :",caseNumber);
            return caseNumber;
        }
    }
    while(js8ArrayNumbers.length<8){    // dizi uzunluğuna göre tekrarlar
